...
def forward_result(dim,x,g1,g2,g3,cd,horizontal,reverse):
	batch = dim[0]
	channel = dim[1]
	height = dim[2]
	width = dim[3]
	...
	if horizontal and (not reverse):
			...
			for i in range(0, height):
				for c in range(0, channel):
					for n in range(0, batch):
						h[n,c,i,j] = get_data(n,c,i,j,cd)*get_data(n,c,i,j,x) + (get_gate(n,c,i,j,i-1,j-1,g1)*get_data(n,c,i-1,j-1,h) + get_gate(n,c,i,j,i,j-1,g2)*get_data(n,c,i,j-1,h) + get_gate(n,c,i,j,i+1,j-1,g3)*get_data(n,c,i+1,j-1,h))*(1-get_data(n,c,i,j,cd))
		return h
		...
		for i in range(0, height):
			for j in range(0, width):
				for c in range(0, channel):
					for n in range(0, batch):
						h[n,c,i,j] = get_data(n,c,i,j,cd)*get_data(n,c,i,j,x) + (get_gate(n,c,i,j,i-1,j-1,g1)*get_data(n,c,i-1,j-1,h) + get_gate(n,c,i,j,i-1,j,g2)*get_data(n,c,i-1,j,h) + get_gate(n,c,i,j,i-1,j+1,g3)*get_data(n,c,i-1,j+1,h))*(1-get_data(n,c,i,j,cd))
		return h
	else:
...
def check_pass(x1, x2, y1, y2):
	if int(x1) == 0 and int(x2) == 0 and (np.absolute(y1)-np.absolute(y2))/(np.absolute(y1)+np.absolute(y2)) < 0.05:
...
for i in range(0, dim_iter):
	...
	scn = mx.sym.SCN(X=x, G1=g1, G2=g2, G3=g3, C=c, horizontal=horizontal, reverse=reverse ,name='scn')
	mod = mx.mod.Module(scn, data_names=['X', 'G1', 'G2', 'G3', 'C'], context=[mx.gpu(0)], label_names=None)
	mod.bind(data_shapes=[('X', dim),('G1', dim),('G2', dim),('G3', dim),('C', dim)],inputs_need_grad=True)
	mod.init_params()
	...
	c = np.random.randint(2,size=dim)
	x = np.multiply(x,c)

	if test_cpu:
		h_py = forward_result(dim,x,g1,g2,g3,c,horizontal,reverse)
		h_py = np.sum(h_py)
	...
	for j in range(0, check_iter):
		...
		if check_var == 'g1':
			Batch = mx.io.DataBatch(
				data=[x_m, g1_m, g2_m, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)

			Batch_plus = mx.io.DataBatch(
				data=[x_m, check_m_plus, g2_m, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)

			Batch_minus = mx.io.DataBatch(
				data=[x_m, check_m_minus, g2_m, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)
			...
			Batch_plus = mx.io.DataBatch(
				data=[x_m, g1_m, check_m_plus, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)

			Batch_minus = mx.io.DataBatch(
				data=[x_m, g1_m, check_m_minus, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)
		elif check_var == 'g3':
			Batch = mx.io.DataBatch(
			...
			Batch_plus = mx.io.DataBatch(
				data=[x_m, g1_m, g2_m, check_m_plus, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)

			Batch_minus = mx.io.DataBatch(
				data=[x_m, g1_m, g2_m, check_m_minus, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)
			...
			Batch_plus = mx.io.DataBatch(
				data=[check_m_plus, g1_m, g2_m, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)

			Batch_minus = mx.io.DataBatch(
				data=[check_m_minus, g1_m, g2_m, g3_m, c_m],
				provide_data=[('X', dim), ('G1', dim), ('G2', dim), ('G3', dim), ('C', dim)]
			)
